...
class ShopifyUpload implements ShouldQueue
{
    ...
    //     // $prefix = Shopify::where('vendor_id', $this->data['vendor_id'])->first();
    ...
    //             $order_no = $this->shop['order_prefix'] . $value['order_no'];
    ...
    //                     if (!$product) {
    //                         $product = new Product;
    //                         $data = ['vendor_id' => $sale->seller_id, 'product_name' => $product_item['name'], 'price' => $product_item['price']];
    //                         $product = $product->product_create($data, $this->user);
    //                     }
    ...
    //         }
    //         /* 
    //         if ($this->market_place == 'Shopify') {
    //             $shop = Shopify::find($this->shop->id);
    //             $shop->last_order_synced = now();
    //             $shop->save();
    //         } elseif ($this->market_place == 'Woocommerce') {

    //             $shop = Woocommerce::find($this->shop->id);
    //             $shop->last_order_synced = now();
    //             $shop->save();
    //         } */
    //         // $users = User::all();
    //         // $message = 'New orders uploaded by ' . '<b style="color: red">' . $this->user->name . '</b>';
    //         // broadcast(new OrderUploadEvent($this->user, $message))->toOthers();
    //         // Notification::send($users, new OrderUploadNotification($this->user, $message));
    //     }
    // }
    ...
    public function handle()
    {
        ...
        // $warehouse_id = (array_key_exists('warehouse_id', $this->data)) ? $this->data['warehouse_id'] : null;

        // $default_zone = Zone::where('ou_id', $this->shop->ou_id)->where('default_zone', true)->first('id');
            ...
            foreach ($orders as $value) {
                ...
                if (!$sale_exist) {
                    ...
                    if (array_key_exists('country', $value)) {
                        if ($value['country']) {
                            $country_val = Ou::where('ou_code', $value['country'])->first();
                            if ($country_val) {
                                $ou_id = $country_val->id;

                                if ($ou_id === 1 && $this->shop->ou_id === 4) {
                                    $ou_id = 4;
                                }
                            } else {
                                $ou_id = $this->shop->ou_id;
                    ...
                    $sale->status = $status;
                    $sale->delivery_date = (array_key_exists('delivery_date', $value)) ? $value['delivery_date'] : null;
                    $sale->checkout_id = (array_key_exists('checkout_id', $value)) ? $value['checkout_id'] : null;
