...
class GoogleSyncJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $id = $this->id;
        $sheets = Sheet::where('active', true)->with(['vendor'])->find($id);
        ...
        $orders = Sale::whereIn('order_no', $filteredArray)->where('seller_id', $vendor_id)
            ...
            $order->client_city = ($order->client) ? $order->client->city : null;
            ...
            try {
                foreach ($orders as $order) {
                    ...
                    if ($city) {
                        ...
                        $range_4 = $this->getRangeByOrderNumber($values, $order_values, $orderNumber, $sheet_name, 'City');
                        if ($range_4) {
                            $data[] = new ValueRange([
                                'range' => $range_4,
                                'values' => [$updatedData_4],
                            ]);
                        }
                    }
                    ...
                    if ($client_name) {
                        ...
                        $range_5 = $this->getRangeByOrderNumber($values, $order_values, $orderNumber, $sheet_name, 'Client Name');
                        if ($range_5) {
                            $data[] = new ValueRange([
                                'range' => $range_5,
                                'values' => [$updatedData_5],
                            ]);
                        }
                    }
