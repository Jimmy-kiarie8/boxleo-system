...
class BackupDatabase extends Command
{
    ...
    public function handle()
    {
        try {
            ...
            // Store the backup in DigitalOcean Spaces
            Storage::disk('spaces')->put('boxleo/backups/' . $filename, $backup, 'private');

            Storage::disk('google')->put($filename, $backup);

        } catch (\Throwable $th) {
            //throw $th;
            Mail::raw('There has been an error backing up the database.', function ($message) {
                $message->to(env('ADMIN_MAIL', env('ADMIN_MAIL', 'support@logixsaas.com')), "Logixsaas")->subject("Backup Error");
            });
        }
        return;
        /*try {
