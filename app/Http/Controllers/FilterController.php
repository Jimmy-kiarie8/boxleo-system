...
class FilterController extends Controller
{
    public function sale_filter(Request $request)
    {
        ...
        })->orderBy('id', 'DESC')->withSum('shipcharges', 'amount');
        ...
        if ($status == 'Open') {
            ...
            $sales = $sales->whereNotIn('delivery_status', $statuses)->whereDate('recall_date', Carbon::today())->paginate(200);
        } elseif ($status == 'Pending') {
            $statuses = ['Delivered', 'Dispatched', 'Scheduled', 'Returned', 'Cancelled', 'In Transit', 'New', 'Awaiting Dispatch', 'Undispatched'];
            $sales = $sales->whereNotIn('delivery_status', $statuses)->paginate(200);
        }  elseif ($status == 'Out Of Stock') {
            $statuses = ['Out Of Stock', 'out_of_stock', 'out-of-stock'];
            $sales = $sales->whereIn('delivery_status', $statuses)->paginate(200);
        }elseif ($status == 'Returned') {
            $sales = $sales->where('delivery_status', $status)->paginate(200);
        ...
        // $sales_count = $this->status_counts($sale_arr);

        // return $sale_trans->trans_sales($sales, 'custom', $app_custom_id);
    ...
    public function status_counts($sales)
    {
        ...
        //     $sales_item = $sales;
        //     if ($status == 'Open') {
        //         $data[$status] = $sales_item->where('status', 'Open')->count();
        //     }
        // }
        return $data;
    }
