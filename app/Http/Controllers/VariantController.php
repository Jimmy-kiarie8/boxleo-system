...
class VariantController extends Controller
{
    ...
    public function product_variant($id)
    {
        ...
        $t_a = [];
        foreach ($attribute_arr as $key => $element_art) {
            // return ($element_art['tags']);
            // if (count($element_art['tags']) < 1) {
            //     Arr::forget($attribute_arr, $art_index);
            // }
            $dataList  = collect( $element_art['tags'] )->unique();

            $attribute_arr[$key]['tags'] =$dataList;

            // return $collection = ->map(function ($array) {
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($variants_sku as $sku) {
            ...
            // $sku_no = $product->product_name
            // .substring(0, 2) + '_' + element_b.option_name.substring(0, 2) + '_' + element_a.option_name.substring(0, 2)
