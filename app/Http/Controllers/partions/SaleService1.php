...
class SaleService1
{
    ...
    {
        // Define partition ranges (adjust as needed)
        $partitionRanges = [
            [
        ...
        foreach ($partitionRanges as $range) {
            Log::info('Processing partition: ' . $range['start']->format('Y-m-d') . ' - ' . $range['end']->format('Y-m-d'));
            $query = Sale::with([
            ...
            // Use table hint for partition scanning
            $query->from(DB::raw('`sales` FORCE INDEX (PRIMARY)'));

            // Apply date range filter for the current partition
            $query->whereBetween(DB::raw('UNIX_TIMESTAMP(created_at)'), [
                $range['start']->timestamp,
                $range['end']->timestamp
            ]);

            // Apply search criteria based on $data_arr
            if (count($data_arr) > 1) {
                $query->where(function ($query) use ($data_arr) {
                    foreach ($data_arr as $search) {
            ...
            // Apply additional filter based on authenticated user's organization
            if (Auth::check()) {
                $query->withoutGlobalScope(OrderScope::class)
                    ->where('ou_id', Auth::user()->ou_id);
            }

            // Execute the query to check if results exist
            if ($query->exists()) {
                Log::info('Results found in partition: ' . $range['start']->format('Y-m-d') . ' - ' . $range['end']->format('Y-m-d'));
                $finalQuery = $query;
                break; // Stop searching if results are found in a partition
            }
        }
        // Log::info('Final query: ' . $finalQuery->toSql());

        // If no results were found in any partition, return an empty query
        return $finalQuery ? $finalQuery->orderBy('id', 'DESC') : Sale::whereRaw('0 = 1');
    }


    // Add a new method to allow custom date ranges that respects partitioning
    public function setDateRange($query, $startDate, $endDate)
    {
        return $query->whereBetween(DB::raw('UNIX_TIMESTAMP(created_at)'), [
