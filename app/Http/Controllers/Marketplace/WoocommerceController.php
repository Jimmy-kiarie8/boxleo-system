...
class WoocommerceController extends Controller
{
    ...
    public function woocommerce_auth(Request $request, $vendor_id)
    {
        // return $request->all();

        $store_url = 'https://' . $request->woocommerce_url;
        // $store_url = 'https://woocommerce-317638-2025580.cloudwaysapps.com';
        $endpoint = '/wc-auth/v1/authorize';
        // $url = $this->remove_http($store_url);
        $url = $store_url;

        $data = ['store_url' => $url, 'vendor' => $vendor_id];

        // $encrypt = Crypt::encrypt($data);
        $encrypt = base64_encode(serialize($data));
        $callback_url =  env('APP_URL') . '/api/woocommerce_callback?store=' . $encrypt;
        // return $callback_url =  env('APP_URL') . '/api/woocommerce_callback/' . $encrypt;

        $params = [
            'app_name' => 'MFT',
            'scope' => 'read_write',
            'user_id' => 123,
            'return_url' =>  env('APP_URL') . '/api/woocommerce_return',
    ...
    public function sync_orders(Request $request)
    {
        ...
        // $woo_orders = $woo_orders->orders('', '', $vendor_id, $woocommerce->id);
    ...
    public function plugin_orders(Request $request)
    {
        $vendor_id = $request->client_code;
        $woocommerce = Woocommerce::where('vendor_id', $vendor_id)->first();
        $woo_orders = ['orders' => $request->all()];
        $order = new Woocommerce();
        $sales = $order->transform_plugin($woo_orders, $vendor_id);
        $data = ['orders' => $sales, 'vendor_id' => $vendor_id];
        $user = User::first();
        WoocommerceJob::dispatch($data, $user, $woocommerce, 'Woocommerce');
    }
}
