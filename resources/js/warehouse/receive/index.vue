    ...
    <v-stepper v-model="e6" vertical style="width: 70%;margin: auto">
            ...
            <v-card class="mb-12" elevation="3" style="padding: 10px">
                <v-layout row wrap>
                    ...
                    <v-flex sm12 v-if="form.vendor_id">
                        ...
                        <el-select v-model="form.products" placeholder="Select" multiple clearable filterable
        ...
        <v-stepper-content step="2">
            <v-card class="mb-12" elevation="3" style="padding: 10px">
                ...
                <v-card-text>
                    <template>
                        <v-data-table :headers="headers" :items="form.products" :items-per-page="5" class="elevation-1"
                            :search="search">


                            <template v-slot:item.quantity="{ item }">
                                <el-input placeholder="Qty" v-model="item.quantity"></el-input>
                            </template>
        ...
        <v-stepper-content step="3">
            ...
            <v-card class="mb-12" elevation="3" style="padding: 10px">
                <v-card-title primary-title>
                    <!-- <v-btn color="primary" text @click="bin_check">Check for bins</v-btn> -->
                    <v-btn color="primary" text :href="'/pickup_note/' + form_id" target="_blank">Print Clearance
                        form</v-btn>
                    <v-btn color="primary" text :href="'/product_sticker/' + form_id" target="_blank">Print product
                        stickers</v-btn>
                </v-card-title>
                <v-card-text>
                    <template>
                        <v-data-table :headers="columns" :items="form.products" :items-per-page="5" class="elevation-1"
                            :search="search">
...
export default {
    ...
    computed: {
        ...mapState(['sellers', 'products', 'bins'])
    },
