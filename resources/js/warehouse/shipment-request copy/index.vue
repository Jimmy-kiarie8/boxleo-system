<template>
    <v-card elevation="4">
        ...
        <v-data-table :headers="headers" :items="shipments.data" :search="search">
            ...
            <template v-slot:item.payment_code="props" v-if="user.can['Order edit']">
                <v-edit-dialog :return-value.sync="props.item.payment_code" large persistent
                    @save="update_item(props.item, 'payment_code')" @cancel="cancel" @open="open_dialog"
                    @close="close">
                    <span id="address_tab">
                        <el-tooltip class="item" effect="dark" :content="props.item.payment_code"
                            placement="top-start">
                            <span>
