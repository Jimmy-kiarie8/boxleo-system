            ...
            <v-flex sm12>
                <v-card style="padding: 20px 0;">
                    <el-breadcrumb separator-class="el-icon-arrow-right">
                        ...
                        <el-breadcrumb-item :to="{ path: '/custom' }">Custom</el-breadcrumb-item>
            ...
            <v-flex sm12>
                <v-card style="padding: 10px 0;">
                    <v-layout row>
                        <v-flex sm1 style="margin-left: 10px;">
                            <v-tooltip right>
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" slot="activator" class="mx-0" @click="getCustoms">
                        ...
                        <v-flex sm2>
                            <h3 style="margin-left: 30px !important;margin-top: 10px;">Customs</h3>
                        </v-flex>
                        <v-flex offset-sm8 sm2>
                            <v-btn color="info" @click="openCreate" text>Create Custom</v-btn>
                        </v-flex>
                    </v-layout>
                </v-card>
            </v-flex>
            <!-- <v-flex sm12>
                <v-pagination v-model="custom.current_page" :length="custom.last_page" total-visible="5" @input="next_page(custom.path, custom.current_page)" circle v-if="custom.last_page > 1"></v-pagination>
            </v-flex> -->
            <v-flex sm12>
                <!-- <vue-good-table class="table-hover" :columns="columns" :rows="app_custom" :search-options="{ enabled: true }" :pagination-options="{enabled: true,mode: 'pages'}" v-loading="loading" :sort-options="{enabled: true, initialSortBy: {field: 'id', type: 'asc'}}">
                ...
                <v-data-table :headers="headers" :items="app_custom" :search="search">
...
export default {
    ...
    methods: {
        openCreate() {
            // eventBus.$emit("openCreateCustom");
        ...
        deleteItem(item) {
            this.$store.dispatch("deleteItem", "custom/" + item.id).then(response => {
                this.$message({
                    type: 'success',
        ...
        openShow(data) {
            eventBus.$emit("openShowCustom", data);
        },
        getCustoms() {
            var payload = {
                model: 'app_custom',
        ...
        next_page(path, page) {
            var payload = {
                path: path,
                page: page,
                update: 'updateCustomsList'
            }
            this.$store.dispatch("nextPage", payload);
        },
    },
    computed: {
        ...mapState(['app_custom', 'loading'])
    },
    mounted() {
        // this.$store.dispatch('getCustoms');
