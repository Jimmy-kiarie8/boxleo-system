...
<v-card>
    ...
    <v-card-text>
        <el-collapse accordion>
            ...
            <el-collapse-item name="1" v-if="shopify_orders.exist_orders.length > 0">
                <template slot="title" style="color: red">
                    <span><b style="color: red">{{ shopify_orders.exist_orders.length }} Orders Exists</b><i class="header-icon el-icon-info"></i></span>
                </template>
                <div>
                    <v-list dense two-line>
                        <v-list-item-group color="primary">
                            <v-list-item v-for="(item, i) in shopify_orders.exist_orders" :key="i">
                                <v-list-item-content>
        ...
        <v-data-table :headers="headers" :items="shopify_orders.sales" :search="search" :loading="loading">
            <!-- <v-data-table :headers="headers" :items="shopify_orders" :search="search" :item-class="itemRowBackground"> -->
...
export default {
    ...
    methods: {
        ...
        remove(item) {
            const index = this.shopify_orders.sales.indexOf(item)
            this.shopify_orders.sales.splice(index, 1)
        },
    },
    computed: {
