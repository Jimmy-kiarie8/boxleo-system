            ...
            <v-flex sm12>
                <v-card style="padding: 10px 0;">
                    <v-layout row>
                        <v-flex sm1 style="margin-left: 10px;">
                            <v-tooltip right>
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" slot="activator" class="mx-0" @click="getCategories">
                        ...
                        <v-flex sm2>
                            <h3 style="margin-left: 30px !important;margin-top: 10px;">Categories</h3>
                        </v-flex>
                        <v-flex offset-sm8 sm2>
                            <v-btn color="info" @click="openCreate" text>Create Category</v-btn>
                        </v-flex>
                    </v-layout>
                </v-card>
            </v-flex>
            <v-flex sm12>
                <v-pagination v-model="categories.current_page" :length="categories.last_page" total-visible="5" @input="next_page(categories.path, categories.current_page)" circle v-if="categories.last_page > 1"></v-pagination>
            </v-flex>
            <v-flex sm12>
                <!-- <vue-good-table class="table-hover" :columns="columns" :rows="categories.data" :search-options="{ enabled: true }" :pagination-options="{enabled: true,mode: 'pages'}" v-loading="isLoading" >
                ...
                <v-data-table :headers="headers" :items="categories.data" :search="search">
...
export default {
    ...
    methods: {
        openCreate() {
            eventBus.$emit("openCreateCategory");
            this.getMenu()

        },
        openEdit(data) {
            eventBus.$emit("openEditCategory", data);
            this.getMenu()
        },
        ...
        deleteItem(item) {
            this.$store.dispatch("deleteItem", "categories/" + item.id).then(response => {
                this.$message({
                    type: 'success',
        ...
        openShow(data) {
            eventBus.$emit("openShowCategory", data);
        },
        getCategories() {
    ...
    created() {
        eventBus.$on("categoryEvent", data => {
            this.getCategories();
        });
        ...
        eventBus.$on("responseChooseEvent", data => {
            console.log(data);
            if (data == "Excel") {
                eventBus.$emit("openEditCategory");
            } else {
                eventBus.$emit("openCreateCategory");
            }
        });
    ...
    //     next(vm => {
    //       if (vm.user.can["view categories"]) {
    //         next();
    //       } else {
    //         next("/");
    //       }
