...
<v-card>
    ...
    <v-card-text v-if="e1 == 2">
        ...
        <v-data-table :headers="headers" :items="sellers.sellers" :search="search" :loading="loading">
            <!-- <v-data-table :headers="headers" :items="sellers" :search="search" :item-class="itemRowBackground"> -->
            <!-- <div :style="[ (!seller.seller.id || !seller.entry.sellerid || !seller.entry.quantity || !seller.entry.address || !seller.entry.sellername || !seller.entry.phone) ? {'background': '#ff00008a'} : '']"> -->
...
export default {
    ...
    computed: {
        ...
        //         if (!element.seller.id || !element.entry.sellerid || !element.entry.quantity || !element.entry.address || !element.entry.sellername || !element.entry.phone) {
        //             valid = false
        //             break;
        //         } else {
        //             valid = true
