            ...
            <v-flex sm12>
                <v-card style="padding: 10px 0;">
                    <v-layout row>
                        <v-flex sm1 style="margin-left: 10px;">
                            <v-tooltip right>
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" slot="activator" class="mx-0" @click="getBrands">
                        ...
                        <v-flex sm2>
                            <h3 style="margin-left: 30px !important;margin-top: 10px;">Brands</h3>
                        </v-flex>
                        <v-flex offset-sm8 sm2>
                            <v-btn color="info" @click="openCreate" text>Create Brand</v-btn>
                        </v-flex>
                    </v-layout>
                </v-card>
            </v-flex>
            <v-flex sm12>
                <v-pagination v-model="brands.current_page" :length="brands.last_page" total-visible="5" @input="next_page(brands.path, brands.current_page)" circle v-if="brands.last_page > 1"></v-pagination>
            </v-flex>
            <v-flex sm12>
                <!-- <vue-good-table class="table-hover" :columns="columns" :rows="brands.data" :search-options="{ enabled: true }" :pagination-options="{enabled: true,mode: 'pages'}" v-loading="isLoading" :sort-options="{enabled: true, initialSortBy: {field: 'id', type: 'asc'}}">
                ...
                <v-data-table :headers="headers" :items="brands.data" :search="search">
                        <template v-slot:item.actions="{ item }">
                            ...
                            <v-tooltip bottom>
                                ...
                                <span>Delete {{ item.subcategory }}</span>
                            </v-tooltip> -->
                            <v-tooltip bottom>
                                <template v-slot:activator="{ on }">
                                    <v-btn v-on="on" icon class="mx-0" @click="openEdit(item)" slot="activator">
...
export default {
    ...
    methods: {
        ...
        openEdit(data) {
            eventBus.$emit("openEditBrand", data);
        },
        ...
        deleteItem(item) {
            this.$store.dispatch("deleteItem", "brands/" + item.id).then(response => {
                this.$message({
                    type: 'success',
        ...
        openShow(data) {
            eventBus.$emit("openShowBrand", data);
        },
        getBrands() {
