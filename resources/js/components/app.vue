<template>
    <v-main :style="[($vuetify.theme.dark) ? { 'background': '#1e1e1e' } : '']" style="height: 100vh !important;">
        ...
        <router-view :user="user" :tenant="tenant" v-loading="page_loader" :element-loading-text="app_name + '...'"
...
export default {
    ...
    created() {
        ...
        let client = null;
        let isConnecting = false; // Track if a connection attempt is already in progress

        const initializeClient = () => {
            if (isConnecting) {
                return; // Exit if a connection attempt is already in progress
            }

            client = new Africastalking.Client(this.user.agent_token, params);

            client.on('closed', () => {
                eventBus.$emit('connectionEvent', false);
                console.log('Connection closed. Attempting to reconnect...');
                console.log("🚀 ~ file: app.vue:201 ~ client.on ~ client._connection:", client._connection)

                // Check if the WebSocket connection is in a closing state
                if (client._connection && (client._connection.readyState === WebSocket.CLOSING || client._connection.readyState === WebSocket.CLOSED)) {
                    isConnecting = false; // Set connecting flag to false
                    initializeClient(); // Re-establish the connection
                }
            });
        ...
        client.on('hangup', hangupCause => {
            ...
            this.toggleDialer(false)
            this.toggleAudio(false)
            eventBus.$emit('alertRequest', 'Hanged up: ' + hangupCause.reason)
